...
def shuffle_logits(x, nmb_views, bs):
    x = x.view(bs, nmb_views, -1)
    
    # Generate a random permutation of indices for dimension 1
    perm = torch.randperm(x.shape[1])
    perm = perm.to(x.device)

    # Shuffle the tensor along dimension 1 using the generated permutation
    shuffled_x = x.index_select(1, perm).view(bs * nmb_views, -1)
    
    return shuffled_x

def concat_local_logits(x, nmb_views, bs):
    x = x.view(bs, nmb_views, -1)
    x = torch.mean(x, 1)
    return x
